...
class HangingProtocolService {
  ...
  /** Add a given protocol object as active.  Can optionally specify which
   * implementation of a protocol id is the active one.  This is typically
   * used to allow different choices for the "default" or "mpr" protocols.
   */
  public addActiveProtocol(id: string, protocol?: Protocol): void {
    if (!id) {
   ...
   * @paprams hangingProtocols is a list of protocol id's to set as active.
   *     If a single string is provided, it means only the one protocol is applied,
   *     without checking the conditions on the protocol.
   */
